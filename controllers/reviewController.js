...
exports.createReview = async (req, res) => {
  try {
    if (req.user.role !== "client") {
      return res
        .status(403)
        .json({ message: "Hanya klien yang dapat memberikan review." });
    }
    ...
    res
      .status(500)
      .json({ message: "Terjadi kesalahan saat membuat review", error });
  }
};
...
exports.getReviewById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Data review berhasil diambil", data: review });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Gagal mengambil data review", error: error.message });
  }
};
...
exports.getMyReviews = async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ client_id: client._id }).populate(
      "client_id",
      "nama_lengkap"
    );
...
exports.updateReview = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Review berhasil diperbarui", data: updatedReview });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Gagal memperbarui review", error: error.message });
  }
};
...
exports.updateMyReview = async (req, res) => {
  try {
    ...
    if (!updatedReview) {
      return res
        .status(404)
        .json({ message: "Review tidak ditemukan untuk client ini" });
    }
...
exports.deleteReview = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal menghapus review", error: error.message });
  }
};
